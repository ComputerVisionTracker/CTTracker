...
CompressiveTracker::CompressiveTracker(void)
{
	...
	featureNum = 50;	// number of all weaker classifiers, i.e,feature pool，弱分类器数量
	rOuterPositive = 4;	// radical scope of positive samples，正样本的基的范围
	rSearchWindow = 25; // size of search window，扫描窗口的大小
...
*/
{
	...
	for (int i = 0; i < _numFeature; i++){
		...
		for (int j = 0; j < numRect; j++){
			...
			//weightTemp = (float)pow(-1.0, c)/sqrt(num);  
...
//还有一点：实际上这里采用的不属于真正的harr特征，博客中翻译有误。这里计算的是
	...
	for (int i = 0; i < featureNum; i++){
		for (int j = 0; j < sampleBoxSize; j++){
			...
			for (size_t k = 0; k < features[i].size(); k++){
				...
				tempValue += featuresWeight[i][k] * (_imageIntegral.at<float>(yMin, xMin) + _imageIntegral.at<float>(yMax, xMax) - _imageIntegral.at<float>(yMin, xMax) - _imageIntegral.at<float>(yMax, xMin));
			}
